    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-base text-gray-600">{form?.title}</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                <button className="bg-green-50 hover:bg-green-100 text-green-700 px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center space-x-2 transition-colors text-sm sm:text-base">
                  <Download size={16} />
                  <span>Export</span>
                </button>
                <button 
                  onClick={deleteSubmission}
                  className="bg-red-50 hover:bg-red-100 text-red-700 px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center space-x-2 transition-colors text-sm sm:text-base"
                >
                  <Trash2 size={16} />
                  <span>Delete</span>
                </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pt-4 border-t border-gray-200">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-mono text-xs sm:text-sm text-gray-900">{submission._id.slice(-8)}</span>
              </div>
              <div className="flex items-center space-x-2">
              ...
              {submission.files?.length > 0 && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs sm:text-sm text-gray-600">Files:</span>
                  <span className="text-xs sm:text-sm text-gray-900">{submission.files.length}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
            <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-4 sm:mb-6">Form Responses</h2>
            ...
            <div className="space-y-4 sm:space-y-6">
              {form?.fields.map(f => {
                ...
                return (
                  <div key={f.name} className="border-b border-gray-100 pb-3 sm:pb-4 last:border-b-0 last:pb-0">
          ...
          {submission.files?.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
              <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-4 sm:mb-6">Attached Files</h2>
                  ...
                  <div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 bg-gray-50 rounded-lg space-y-2 sm:space-y-0">
                    ...
                    {f.url && (
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors text-center sm:text-left"
                      >
                        Download
                      </a>
                    )}
